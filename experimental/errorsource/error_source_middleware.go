...
func ErrorSourcenMiddleware() httpclient.Middleware {
	return httpclient.NamedMiddlewareFunc(ErrorSourceMiddlewareName, func(opts httpclient.Options, next http.RoundTripper) http.RoundTripper {
		return httpclient.RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
			res, err := next.RoundTrip(req)
			if res != nil && res.StatusCode >= 400 {
				errorSource := backend.GetErrorSource(res.StatusCode)
				errors.WithLabelValues(string(errorSource)).Inc()
			}
			return res, err
		})
	})
