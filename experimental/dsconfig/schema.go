// Package dsconfig is a declarative datasource configuration schema. It is the
// single source of truth for a datasource's configuration: every field declares
// its storage location (root / jsonData / secureJsonData), type, and validation.
// ToSettings converts it to the experimental/pluginschema Settings shape (see
// convert.go).
package dsconfig
