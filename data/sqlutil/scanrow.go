...
func MakeScanRow(colTypes []*sql.ColumnType, colNames []string, converters ...Converter) (*RowConverter, error) {
	...
	for i, name := range colNames {
		if j, ok := seen[name]; ok {
			return nil, errorsource.WithDownstreamSource(fmt.Errorf(`duplicate column names are not allowed, found identical name "%v" at column indices %v and %v`, name, j, i), false)
		}
		seen[name] = i
	}
