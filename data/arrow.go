...
func (f *Frame) MarshalArrow() ([]byte, error) {
	...
	// Pre-size to avoid bytes.Buffer doubling during IPC write.
	if n := estimateMarshalArrowSize(f); n > 0 {
		fb.Buff.Grow(n)
	}
...
// estimateMarshalArrowSize returns a conservative byte estimate for the Arrow
// IPC serialization of f. Used only to pre-size the output buffer so that
// bytes.Buffer doubling does not dominate allocation for large frames. An
// under-estimate just means a small number of later doublings; the estimate
// does not need to be exact.
func estimateMarshalArrowSize(f *Frame) int {
	if f == nil || len(f.Fields) == 0 {
		return 0
	}
	rows := 0
	if len(f.Fields) > 0 && f.Fields[0] != nil {
		rows = f.Fields[0].Len()
	}
	// Baseline schema + IPC framing overhead (empirically ~1–2 KB even for
	// tiny frames; a bit higher when labels/config are present).
	...
	for _, fld := range f.Fields {
		...
		// Per-field metadata (labels/config JSON in the schema block).
		total += 128
		if len(fld.Labels) > 0 {
		...
		switch fld.Type() {
		case FieldTypeInt8, FieldTypeNullableInt8, FieldTypeUint8, FieldTypeNullableUint8, FieldTypeBool, FieldTypeNullableBool:
			total += rows + rows/8
		case FieldTypeInt16, FieldTypeNullableInt16, FieldTypeUint16, FieldTypeNullableUint16, FieldTypeEnum, FieldTypeNullableEnum:
			total += rows*2 + rows/8
		case FieldTypeInt32, FieldTypeNullableInt32, FieldTypeUint32, FieldTypeNullableUint32, FieldTypeFloat32, FieldTypeNullableFloat32:
			total += rows*4 + rows/8
		case FieldTypeInt64, FieldTypeNullableInt64, FieldTypeUint64, FieldTypeNullableUint64, FieldTypeFloat64, FieldTypeNullableFloat64, FieldTypeTime, FieldTypeNullableTime:
			total += rows*8 + rows/8
		case FieldTypeString, FieldTypeNullableString:
			// Arrow stores 4-byte offsets + variable-length data. Assume an
			// average of 16 bytes/value (low enough to rarely over-shoot on
			// short-string datasets, high enough that string-heavy frames
			// avoid most doublings).
			total += rows*(4+16) + rows/8
		case FieldTypeJSON, FieldTypeNullableJSON:
			total += rows*(4+32) + rows/8
		default:
			total += rows * 8
		}
	}
...
func parseColumn(col arrow.Array, i int, nullable []bool, frame *Frame) error {
	...
	case arrow.STRING:
		...
		if nullable[i] {
			if nvec, ok := frame.Fields[i].vector.(*nullableGenericVector[string]); ok {
				for rIdx := 0; rIdx < col.Len(); rIdx++ {
					if v.IsNull(rIdx) {
						nvec.AppendTyped(nil)
						continue
					}
					rv := v.Value(rIdx)
					nvec.AppendTyped(&rv)
			...
			if gvec, ok := frame.Fields[i].vector.(*genericVector[string]); ok {
				for rIdx := 0; rIdx < col.Len(); rIdx++ {
					gvec.AppendTyped(v.Value(rIdx))
				}
			} else {
				for rIdx := 0; rIdx < col.Len(); rIdx++ {
	...
	case arrow.BOOL:
		...
		if nullable[i] {
			if nvec, ok := frame.Fields[i].vector.(*nullableGenericVector[bool]); ok {
			...
			if gvec, ok := frame.Fields[i].vector.(*genericVector[bool]); ok {
				for sIdx := 0; sIdx < col.Len(); sIdx++ {
					gvec.AppendTyped(v.Value(sIdx))
				}
			} else {
				for sIdx := 0; sIdx < col.Len(); sIdx++ {
	...
	case arrow.BINARY:
		...
		if nullable[i] {
			if nvec, ok := frame.Fields[i].vector.(*nullableGenericVector[json.RawMessage]); ok {
			...
			if gvec, ok := frame.Fields[i].vector.(*genericVector[json.RawMessage]); ok {
				for sIdx := 0; sIdx < v.Len(); sIdx++ {
					gvec.AppendTyped(json.RawMessage(v.Value(sIdx)))
				}
			} else {
				for sIdx := 0; sIdx < v.Len(); sIdx++ {
					r := json.RawMessage(v.Value(sIdx))
...
func UnmarshalArrowFrame(b []byte) (*Frame, error) {
	...
	// Estimate total capacity from the first record batch. Files produced by
	// this SDK's MarshalArrow use a single batch (NewTableReader with chunkSize=-1),
	// so this is exact. For files with non-uniform batch sizes the estimate is
	// approximate and append-growth handles the tail.
	firstRec, err := fR.Read()
	if errors.Is(err, io.EOF) {
		if _, err := initializeFrameFields(schema, frame, 0); err != nil {
			return nil, err
		}
		return frame, nil
	}
	...
	capacity := int(firstRec.NumRows()) * fR.NumRecords()
	nullable, err := initializeFrameFields(schema, frame, capacity)
	if err != nil {
		firstRec.Release()
		return nil, err
	...
	if err := populateFrameFieldsFromRecord(firstRec, nullable, frame); err != nil {
		firstRec.Release()
		return nil, err
	}
	firstRec.Release()
