// Package dsql provides helpers for scanning database/sql responses into
// a data.Frame.
// It is named "dsql" so the name will not collide with a database/sql import.
package dsql
...
func NewFromSQLRows(rows *sql.Rows, converters ...SQLStringConverter) (*data.Frame, map[int]SQLStringConverter, error) {
...
func newForSQLRows(rows *sql.Rows, converters ...SQLStringConverter) (*data.Frame, map[int]SQLStringConverter, error) {
