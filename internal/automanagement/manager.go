...
func (m *Manager) ConvertObjects(ctx context.Context, req *backend.ConversionRequest) (*backend.ConversionResponse, error) {
	h, err := m.Get(ctx, req.PluginContext)
	if err != nil {
		return nil, err
	}
	if ds, ok := h.(backend.ConversionHandler); ok {
		return ds.ConvertObjects(ctx, req)
	}
	return nil, status.Error(codes.Unimplemented, "unimplemented")
}
