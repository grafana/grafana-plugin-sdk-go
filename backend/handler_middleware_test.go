...
func TestHandlerFromMiddlewares(t *testing.T) {
	...
	c := &handlertest.Handler{
		...
		ConvertObjectFunc: func(_ context.Context, _ *backend.ConversionRequest) (*backend.ConversionResponse, error) {
	...
	_, _ = d.ConvertObject(context.Background(), &backend.ConversionRequest{})
