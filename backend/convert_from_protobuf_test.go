...
func TestConvertFromProtobufQueryMigrationRequest(t *testing.T) {
	protoReq := &pluginv2.QueryMigrationRequest{
		PluginContext: protoPluginContext,
		Queries: []*pluginv2.DataQuery{
			protoDataQuery,
		},
	...
	protoWalker := &walker{}
	err := reflectwalk.Walk(protoReq, protoWalker)
	require.NoError(t, err)

	if protoWalker.HasZeroFields() {
		t.Fatalf(unsetErrFmt,
			"proto", "QueryMigrationRequest", protoWalker.ZeroValueFieldCount, protoWalker.FieldCount)
	}

	sdkReq := f.QueryMigrationRequest(protoReq)

	sdkWalker := &walker{}
	err = reflectwalk.Walk(sdkReq, sdkWalker)
	require.NoError(t, err)

	if sdkWalker.HasZeroFields() {
		t.Fatalf(unsetErrFmt, "sdk", "QueryMigrationRequest", sdkWalker.ZeroValueFieldCount, sdkWalker.FieldCount)
	}
	...
	// PluginContext
	requireCounter.Equal(t, protoReq.PluginContext.OrgId, sdkReq.PluginContext.OrgID)
	requireCounter.Equal(t, protoReq.PluginContext.PluginId, sdkReq.PluginContext.PluginID)
	requireCounter.Equal(t, protoReq.PluginContext.ApiVersion, sdkReq.PluginContext.APIVersion)

	// Queries
	requireCounter.Equal(t, protoReq.Queries[0].RefId, sdkReq.Queries[0].RefID)
	requireCounter.Equal(t, protoReq.Queries[0].MaxDataPoints, sdkReq.Queries[0].MaxDataPoints)
	requireCounter.Equal(t, protoReq.Queries[0].QueryType, sdkReq.Queries[0].QueryType)
	requireCounter.Equal(t, time.Minute, sdkReq.Queries[0].Interval)
	requireCounter.Equal(t, sdkTimeRange.From, sdkReq.Queries[0].TimeRange.From)
	requireCounter.Equal(t, sdkTimeRange.To, sdkReq.Queries[0].TimeRange.To)
	requireCounter.Equal(t, json.RawMessage(protoReq.Queries[0].Json), sdkReq.Queries[0].JSON)
}
