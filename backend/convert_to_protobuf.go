...
func (t ConvertToProtobuf) Permission(permission Permission) *pluginv2.Permission {
	return &pluginv2.Permission{
		Action: permission.Action,
		Scope:  permission.Scope,
	}
}

func (t ConvertToProtobuf) RoleRegistration(reg RoleRegistration) *pluginv2.RoleRegistration {
	res := &pluginv2.RoleRegistration{
		Role: &pluginv2.Role{
			Name:        reg.Role.Name,
			Uid:         reg.Role.UID,
			...
			Permissions: make([]*pluginv2.Permission, len(reg.Role.Permissions)),
		},
		Grants: reg.Grants,
	}
	for i := range reg.Role.Permissions {
		res.Role.Permissions[i] = t.Permission(reg.Role.Permissions[i])
	}
	return res
}
...
func (t ConvertToProtobuf) QueryRolesResponse(resp *QueryRolesResponse) *pluginv2.QueryRolesResponse {
	...
	for i := range resp.Registrations {
		registrations[i] = t.RoleRegistration(resp.Registrations[i])
	}
