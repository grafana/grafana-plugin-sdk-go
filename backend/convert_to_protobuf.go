...
func (t ConvertToProtobuf) PluginContext(pluginCtx PluginContext) *pluginv2.PluginContext {
	return &pluginv2.PluginContext{
		...
		CallbackServerID:           pluginCtx.CallbackServerID,
	}
}
...
func (t ConvertToProtobuf) Evaluator(ev Evaluator) *anypb.Any {
	if ev == nil {
		return nil
	}
...
// HasAccessRequest converts the SDK version of a HasAccessRequest to the protobuf version.
func (t ConvertToProtobuf) HasAccessRequest(req *HasAccessRequest) *pluginv2.HasAccessRequest {
	if req == nil {
		return nil
	}

	ev := &pluginv2.Evaluator{
		Ev: t.Evaluator(req.Evaluator),
	}
	return &pluginv2.HasAccessRequest{
		User:      ToProto().User(req.User),
		Evaluator: ev,
	}
}

// HasAccessResponse converts the SDK version of a HasAccessResponse to the protobuf version.
func (t ConvertToProtobuf) HasAccessResponse(res *HasAccessResponse) *pluginv2.HasAccessResponse {
	if res == nil {
		return nil
	}

	return &pluginv2.HasAccessResponse{
