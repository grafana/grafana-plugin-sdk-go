...
func ExampleNewProvider() {
	provider := httpclient.NewProvider(httpclient.CustomHeadersMiddleware())
	log.Printf("Middleware length: %d", len(provider.Middlewares))
}

func ExampleDefaultProvider_New() {
	provider := httpclient.NewProvider([]httpclient.Middleware{
...
func ExampleDefaultProvider_GetTLSConfig() {
	...
	tlsConfig, err := provider.GetTLSConfig(&httpclient.TLSOptions{
