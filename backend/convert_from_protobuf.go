...
// Evaluator converts protobuf version of a Evaluator to the SDK version.
func (f ConvertFromProtobuf) Evaluator(proto *anypb.Any) Evaluator {
	if proto == nil {
		return nil
	}

	switch {
	case proto.MessageIs(&pluginv2.PermissionEvaluator{}):
		perm := &pluginv2.PermissionEvaluator{}
		// TODO think of this error
		_ = proto.UnmarshalTo(perm)
		return PermissionEvaluator{Action: perm.GetAction(), Scopes: perm.GetScopes()}
	case proto.MessageIs(&pluginv2.AnyEvaluator{}):
		any := &pluginv2.AnyEvaluator{}
		// TODO think of this error
		_ = proto.UnmarshalTo(any)
		if len(any.AnyOf) == 0 {
			return nil
		}
		anyOf := []Evaluator{}
		for _, src := range any.AnyOf {
			dst := f.Evaluator(src)
			if dst == nil {
				// TODO think of this case
				continue
			}
			anyOf = append(anyOf, dst)
		}
		return AnyEvaluator{AnyOf: anyOf}
	case proto.MessageIs(&pluginv2.AllEvaluator{}):
		any := &pluginv2.AllEvaluator{}
		// TODO think of this error
		_ = proto.UnmarshalTo(any)
		if len(any.AllOf) == 0 {
			return nil
		...
		allOf := []Evaluator{}
		for _, src := range any.AllOf {
			dst := f.Evaluator(src)
			if dst == nil {
				// TODO think of this case
				continue
			}
			allOf = append(allOf, dst)
		}
...
// HasAccessRequest converts protobuf version of a HasAccessRequest to the SDK version.
func (f ConvertFromProtobuf) HasAccessRequest(req *pluginv2.HasAccessRequest) *HasAccessRequest {
	if req == nil {
		return nil
	}
	var ev Evaluator
	if req.Evaluator != nil {
		ev = f.Evaluator(req.Evaluator.Ev)
	}
	return &HasAccessRequest{
		User:      f.User(req.User),
		Evaluator: ev,
	}
}

// HasAccessResponse converts protobuf version of a HasAccessResponse to the SDK version.
func (f ConvertFromProtobuf) HasAccessResponse(res *pluginv2.HasAccessResponse) *HasAccessResponse {
	if res == nil {
		return nil
	}
