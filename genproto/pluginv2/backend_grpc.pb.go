...
func (c *resourceClient) CallResource(ctx context.Context, in *CallResourceRequest, opts ...grpc.CallOption) (Resource_CallResourceClient, error) {
	...
	x := &resourceCallResourceClient{ClientStream: stream}
...
func _Resource_CallResource_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ResourceServer).CallResource(m, &resourceCallResourceServer{ServerStream: stream})
}
...
func (c *streamClient) RunStream(ctx context.Context, in *RunStreamRequest, opts ...grpc.CallOption) (Stream_RunStreamClient, error) {
	...
	x := &streamRunStreamClient{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
func _Stream_RunStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(StreamServer).RunStream(m, &streamRunStreamServer{ServerStream: stream})
}
...
func (c *admissionControlClient) ConvertObject(ctx context.Context, in *ConversionRequest, opts ...grpc.CallOption) (*ConversionResponse, error) {
	...
	err := c.cc.Invoke(ctx, AdmissionControl_ConvertObject_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
const (
	QueryMigrationControl_MigrateQuery_FullMethodName = "/pluginv2.QueryMigrationControl/MigrateQuery"
)

// QueryMigrationControlClient is the client API for QueryMigrationControl service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type QueryMigrationControlClient interface {
	// Migrates a query
	MigrateQuery(ctx context.Context, in *QueryMigrationRequest, opts ...grpc.CallOption) (*QueryMigrationResponse, error)
}

type queryMigrationControlClient struct {
	cc grpc.ClientConnInterface
}

func NewQueryMigrationControlClient(cc grpc.ClientConnInterface) QueryMigrationControlClient {
	return &queryMigrationControlClient{cc}
}

func (c *queryMigrationControlClient) MigrateQuery(ctx context.Context, in *QueryMigrationRequest, opts ...grpc.CallOption) (*QueryMigrationResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(QueryMigrationResponse)
	err := c.cc.Invoke(ctx, QueryMigrationControl_MigrateQuery_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// QueryMigrationControlServer is the server API for QueryMigrationControl service.
// All implementations should embed UnimplementedQueryMigrationControlServer
// for forward compatibility
type QueryMigrationControlServer interface {
	// Migrates a query
	MigrateQuery(context.Context, *QueryMigrationRequest) (*QueryMigrationResponse, error)
}

// UnimplementedQueryMigrationControlServer should be embedded to have forward compatible implementations.
type UnimplementedQueryMigrationControlServer struct {
}

func (UnimplementedQueryMigrationControlServer) MigrateQuery(context.Context, *QueryMigrationRequest) (*QueryMigrationResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method MigrateQuery not implemented")
}

// UnsafeQueryMigrationControlServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to QueryMigrationControlServer will
// result in compilation errors.
type UnsafeQueryMigrationControlServer interface {
	mustEmbedUnimplementedQueryMigrationControlServer()
}

func RegisterQueryMigrationControlServer(s grpc.ServiceRegistrar, srv QueryMigrationControlServer) {
	s.RegisterService(&QueryMigrationControl_ServiceDesc, srv)
}

func _QueryMigrationControl_MigrateQuery_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(QueryMigrationRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(QueryMigrationControlServer).MigrateQuery(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: QueryMigrationControl_MigrateQuery_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(QueryMigrationControlServer).MigrateQuery(ctx, req.(*QueryMigrationRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// QueryMigrationControl_ServiceDesc is the grpc.ServiceDesc for QueryMigrationControl service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var QueryMigrationControl_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "pluginv2.QueryMigrationControl",
	HandlerType: (*QueryMigrationControlServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "MigrateQuery",
			Handler:    _QueryMigrationControl_MigrateQuery_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "backend.proto",
